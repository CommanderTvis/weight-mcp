...
def test_dashboard_cookie_roundtrip(db: Database) -> None:
    provider = make_provider(db)
    cookie = provider.dashboard_cookie()
    assert provider.dashboard_cookie_valid(cookie)
    assert not provider.dashboard_cookie_valid("garbage")
    # An access token must not pass as a dashboard cookie (typ mismatch).
    assert not provider.dashboard_cookie_valid(provider._issue("c1").access_token)
    # Rotating the password invalidates outstanding cookies.
    assert not make_provider(db, "other").dashboard_cookie_valid(cookie)
